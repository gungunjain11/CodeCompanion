// ModelMirror Vercel Serverless API
...
app.get('/', (_req, res) => {
  res.json({
    status: 'ok',
    message: 'ModelMirror API is running on Vercel!',
    version: '1.1.0',
  });
});
...
app.post('/analyze', async (req, res) => {
  ...
  if (action === 'generate_tests' && !context) {
    return res.status(200).json({
      ...
      context_questions: {
        purpose: 'What is the primary purpose of this code?',
        expected_output: 'What should be the expected output?',
        edge_cases: 'What edge cases should be tested?',
      },
      explanation: 'Please provide context about this code so we can generate meaningful test cases.',
      hint: 'Send another request with the same code and action, but include a "context" object with your answers.',
    });
  }
